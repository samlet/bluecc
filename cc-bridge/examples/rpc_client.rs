...
impl FibonacciRpcClient {
    async fn new() -> Result<Self, lapin::Error> {
        ...
        Ok(Self { conn, channel, callback_queue, consumer, correlation_id })
    }

    async fn call(&mut self, n: u64) -> Result<u64, Box<dyn std::error::Error>> {
        self.channel
            .basic_publish("", "rpc_queue",
                           BasicPublishOptions::default(), n.to_le_bytes().to_vec(),
