...
pub fn extract_val<T: DeserializeOwned>(resp: &SrvResp<DynamicValue>, item_name: &str) -> Result<T, GenericError> {
    if let Some(item) = resp.data.as_ref().unwrap().values.get(item_name) {
        Ok(serde_json::from_value(item.clone())?)
    } else {
        Err(GenericError::NotFound {
            item_name: item_name.to_string(),
            info: "not found item in resp".to_string(),
        })
    }
}
